...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        rows = torch.arange(0.0, max_len).reshape(max_len,-1)
        cols = 10000**(-torch.arange(0.0, embed_dim, 2)/embed_dim)
        x = cols * rows
        pe[:,:,0::2] = torch.sin(x)
        pe[:,:,1::2] = torch.cos(x)
    ...
    def forward(self, x):
        ...
        output = x + self.pe[:,:S,:] # (N, S, D) + (1, max_len, D)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # (1)
        H = self.num_heads
        Q = self.query(query).reshape(N, S, H, D//H).transpose(1,2) # (N, H, S, P)
        K = self.key(key).reshape(N, T, H, D//H).transpose(1,2) # (N, H, T, P)
        V = self.value(value).reshape(N, T, H, D//H).transpose(1,2) # (N, H, T, P)
        # (2)
        E = torch.div(Q.matmul(K.transpose(2, 3)), math.sqrt(D/H)) # (N,H,S,P)x(N,H,P,T).T->(N,H,S,T)  
        # (3)
        if attn_mask is not None:
            E = E.masked_fill(attn_mask == False, -float('inf'))
            
        Y = self.dropout(torch.softmax(E, dim=3)).matmul(V) #(N,H,S,T)x(N,H,T,P)->(N,H,S,P)
